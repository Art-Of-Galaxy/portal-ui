...
import {
  AlertTriangle, ArrowLeft, ArrowRight, Calendar, ChevronLeft,
  Globe, Image as ImageIcon, Loader2, RefreshCw, Sparkles, Upload,
} from "lucide-react";
import { apiServices } from "../../../services/apiServices";
import BrandAssetUploader from "../../../components/brand/BrandAssetUploader";

// WordPress Blog Engine create flow. Mirror of BlogEngineCreate but
// bound to /api/wp-blog-engine/* and /api/wordpress-connections/*.
// Brief / generation / preview UX is identical; only the publish
// step picks a WordPress site + category instead of a Shopify blog.

const MODES = {
  autopilot: {
    emoji: "📰", label: "Autopilot Engine", tag: "Recommended", grad: "linear-gradient(135deg,#21759b,#1e3a5f)",
    desc: "Hand the agent a keyword bank and a cadence. It researches, writes, optimizes, and auto-publishes to WordPress on schedule.",
    pills: ["recurring", "auto-publish", "SEO + GEO + AEO"],
    steps: ["Connecting to your WordPress site", "Clustering your keyword bank", "Drafting the first article", "Optimizing for SEO / GEO / AEO", "Scheduling the publishing queue"],
  },
  single: {
    emoji: "📝", label: "Single Article", tag: "One post", grad: "linear-gradient(150deg,#21759b,#15455c)",
    desc: "One keyword, one polished article. Research, draft, optimize, preview, publish.",
    pills: ["1 article", "SEO-optimized"],
    steps: ["Connecting to your WordPress site", "Researching keyword + search intent", "Outlining the article", "Drafting the full article on-brand", "Optimizing meta, links + schema"],
  },
  cluster: {
    ...
    steps: ["Connecting to your WordPress site", "Mapping pillar + supporting topics", "Drafting the pillar article", "Drafting each supporting post", "Queueing them for publish"],
  },
  bulk: {
    ...
    steps: ["Connecting to your WordPress site", "Parsing your keyword list", "Drafting an article per keyword", "Optimizing each for search", "Queueing them for publish"],
  },
};
...
function TypePicker({ onPick }) {
  return (
    <div className="sm-create-content">
      ...
      <p className="sm-create-sub">Generate SEO / GEO / AEO articles and push them straight to your WordPress site. One at a time, or fully on autopilot.</p>
...
function buildBriefSteps(mode) {
  ...
  const cadenceStep = {
    ...
    sub: "The engine drafts ahead and auto-pushes to WordPress on this cadence.",
...
function Preview({ mode, brief, result, articleId, onFeaturedChange, onScheduleNext, onEdit }) {
  ...
  async function persistFeatured(image_url, source, content_type) {
    ...
    const res = await apiServices.wp_blog_engine_set_featured_image({
      id: articleId,
      image_url,
      source,
      content_type,
  ...
  async function handleFileChosen(e) {
    ...
    try {
      const up = await apiServices.upload_file(file, {
        projectName: brief.brand || "WP Blog Article",
        category: "wp-blog-featured",
        serviceType: "wp-blog-engine",
      });
  ...
  async function handleRegenerate() {
    ...
    try {
      ...
      const res = await apiServices.wp_blog_engine_regen_image({
        id: articleId,
        prompt: imgPrompt,
      ...
      <aside className="be-seo-panel">
        ...
        <section className="be-seo-card">
          ...
          <div className="be-serp-preview">
            <div className="be-serp-url">{(brief.site_url || "yoursite.com")} › {editedHandle || spec.handle || "article"}</div>
          ...
          <div className="be-meta-field">
            <div className="be-meta-flabel"><span>URL slug</span></div>
            <input
              className="be-meta-input"
              value={editedHandle}
        ...
        >
          {mode === "autopilot" ? "Activate autopilot →" : "Publish to WordPress →"}
        </button>
      </aside>
    </div>
  );
...
function Publish({ mode, brief, articleId, connections, onSuccess, onBack }) {
  ...
  const initialConn = brief.wp_connection_id || connections.find((c) => c.is_primary)?.id || connections[0]?.id || null;
  const [wpConnectionId, setWpConnectionId] = useState(initialConn);
  const [categories, setCategories] = useState([]);
  const [categoryId, setCategoryId] = useState(brief.category_id || "");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiServices.wordpress_connections_categories({ id: wpConnectionId });
        if (cancelled) return;
        if (res?.success) {
          setCategories(res.categories || []);
          if (!categoryId && res.categories?.[0]) setCategoryId(res.categories[0].id);
        }
      } catch (err) {
  ...
  }, [wpConnectionId]); // eslint-disable-line react-hooks/exhaustive-deps

  async function handlePublish() {
    if (busy) return;
    if (!wpConnectionId) { setError("Pick a connected site first."); return; }
    setBusy(true);
    setError("");
    try {
      if (isAuto) {
        const keywords = (brief.keywords || "").split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
        const res = await apiServices.wp_blog_engine_autopilot_save({
          wp_connection_id: wpConnectionId,
      ...
      await apiServices.wp_blog_engine_save({
        id: articleId,
        wp_connection_id: wpConnectionId,
      ...
      if (when === "now") {
        await apiServices.wp_blog_engine_publish_now({ id: articleId });
      }
      onSuccess({ when, categoryId });
    } catch (err) {
    ...
    <div className="sm-publish-body">
      <div className="sm-publish-card">
        <div className="sm-publish-head">
          ...
          <div>
            <h2 className="sm-publish-title">{isAuto ? "Activate your Blog Autopilot" : "Publish to WordPress"}</h2>
            <span className="sm-publish-meta">{brief.brand || "Your brand"} · {m.label}</span>
          </div>
        </div>

        <span className="sm-pub-section-label">Publish to which site?</span>
        {connections.length === 0 ? (
          <div className="sm-banner is-warn">
            <AlertTriangle size={14} />
            <span>No connected sites. <a href="/new-projects/ai-integrations/wp-blog/connections">Connect one</a> first.</span>
          </div>
        ) : (
          <div className="sm-platform-grid">
            {connections.map((c) => {
              const sel = wpConnectionId === c.id;
              return (
                <button
                  key={c.id}
                  type="button"
                  className={`sm-platform-card ${sel ? "is-selected" : ""}`}
                  onClick={() => setWpConnectionId(c.id)}
                >
                  <span className="sh-store-badge"><Globe size={14} /></span>
                  <div className="sm-platform-meta">
                    <span className="sm-platform-name">{c.site_name || c.site_url}</span>
                    <span className="sm-platform-sub">{c.site_url}</span>
                  </div>
        ...
        {wpConnectionId && categories.length ? (
          <>
            <span className="sm-pub-section-label">Publish to which category?</span>
            <select
        ...
        <div className="sm-publish-nav">
          ...
          <button type="button" className="sm-publish-btn" onClick={handlePublish} disabled={busy || !wpConnectionId}>
            {busy ? <Loader2 size={14} className="bg-spin" /> : isAuto ? <Sparkles size={14} /> : when === "now" ? <Upload size={14} /> : <Calendar size={14} />}
            {isAuto ? "Activate autopilot" : when === "now" ? "Publish to WordPress" : when === "later" ? "Schedule article" : "Save as draft"}
          </button>
        </div>
      </div>
    </div>
        ...
        <div className="sm-success-confirm">
          ...
          <h2 className="sm-success-title">
            {isAuto ? "Autopilot is live!"
              : info?.when === "now" ? "Published to WordPress!"
                : info?.when === "later" ? "Scheduled to WordPress!"
                  : "Saved as draft"}
          </h2>
          <p className="sm-success-sub">
            {isAuto ? "Your Blog Engine will now write and auto-publish to WordPress on schedule, hands-off."
              : info?.when === "now" ? "Your article is live on your WordPress site."
...
export default function WordPressBlogCreate() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiServices.wp_blog_engine_get({ id: initialArticleId });
        if (cancelled) return;
        ...
        setBrief({
          ...
          wp_connection_id: a.wp_connection_id || null,
        });
        setResult({
          spec: a.spec || {},
  ...
  async function runGeneration() {
    ...
    try {
      ...
      if (mode === "bulk" || mode === "cluster") {
        const keywords = (brief.keywords || "").split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
        res = await apiServices.wp_blog_engine_generate({
          brief: { ...briefForApi, keyword: keywords[0] || briefForApi.keyword },
        });
      } else {
        res = await apiServices.wp_blog_engine_generate({ brief: briefForApi });
      }
      ...
      try {
        const save = await apiServices.wp_blog_engine_save({
          mode,
  ...
  function handleScheduleNext(edits) {
    if (articleId && edits) {
      apiServices.wp_blog_engine_save({
        id: articleId,
  ...
  return (
    <div className="portal-page sm-page">
      ...
      {step === "success" && successInfo ? (
        <Success mode={mode} info={successInfo} onCreateAnother={reset} onHub={() => navigate("/new-projects/ai-integrations/wp-blog")} />
      ) : null}
    </div>
  );
}
